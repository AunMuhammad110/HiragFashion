...
function CheckOut() {
    ...
    const {data,loading,removeFromContext,removeFromCart,TotalCalculator}= useDataContext();
...
useEffect(() => {
         ...
         <div className="container my-4">
                <div className="row d-flex">
                    ...
                    <div className="col-6 d-flex justify-content-center align-items-center pb-4">
                        ...
                        <button type="button" className="checkout-button" onClick={handleChangeOfbtn}>Proceed To checkout</button>
                        </Link>
                    </div>
                    {orderForm && <OrderForm/>}
                    <hr className="w-100"/>
  ...
  data.map((item) => (
    <div key={item._doc.productId} className="row">
      <div className="col-1 mr-2"><img id="logo-img"src={item._doc.images[0] } alt="Product Image"  /></div>
      <div className="col-5">
        <p style={{fontSize:"15px"}}> {item._doc.productId}"   "{item._doc.brandName} {item._doc.subBrandName} {item._doc.productTitle}</p>
      ...
      <div className="col-2">
        <p className="price">
            Rs {item._doc.productPrice*item.quantity}.00</p>
      </div>
      <div className="col-1 justify-icon">
            {/* <p>{item._doc.productId}</p> */}
            <img className="remove-icon"src={remove_icon} alt="remove_i" onClick={(e)=>handleRemoveCartt(item._doc.productId)} />
      </div>
...
: (
  <p>No items added to the cart.</p>
                ...
                <div className="row">
                    ...
                    <div className="col-4">
                <Link to='/context/orderform'>
                        <button type="button" className="checkout-button d-inline my-md-3" onClick={handleChangeOfbtn}>Proceed To checkout</button>
                        </Link>
                    </div>
                ...
                <p className="d-flex justify-content-center mt-3 OrderRes">
                    Hira G's process all orde2rs in PKR. While the content of your cart is currently displayed in PKR,
                     you will check out using PKR at the most current exchange rate.
                </p>
            </div>
  ...
  !smallImagesVisible && (
    <div className="container " style={{paddingLeft:"10px"}}>
      ...
      <div className="row">
            ...
            <React.Fragment key={index}>
              <div className="row seq-ord-1">
                <div className="col-12 pl-3">
                  ...
                  <div className="col-3 d-flex justify-content-center p-1" style={{marginTop:"6px",marginBottom:"6px",padding:"none"}}>
                    <a href="#" className="anchir"> <img className="image" src={item._doc.images[1]} alt="Your Alt Text" style={{maxWidth:"80px",width:"75px",maxHeight:"90px"}}/>      <span className="bg-dark pos" style={{color:"#fff"}}><p style={{display:"flex",justifyContent:"center",alignItems:"center"}}>{item.quantity}</p></span></a>
    </div>
                    <div className="col-6 d-flex flex-column">
                      <div className="">
                        <p>{item._doc.brandName} {item._doc.subBrandName} {item._doc.productTitle}</p>
                      </div>
        ...
        <div className="row pr-4 d-flex justify-content-end" style={
          {marginBottom:"none",height:"27px"}}>
          <p style={
          {marginBottom:"2px"}}>
            <b>Grand Total</b>&nbsp;&nbsp;&nbsp;Rs.{TotalCalculator()}.00
          </p>
        </div>
        <div className="row d-flex justify-content-end pr-4 " style={
          {marginBottom:"none",height:"24px"}
        }>
          <p className="sizeOfTax" style={
          {marginBottom:"2px"}}>Shipping & taxes calculated at checkout</p>
        </div>
        <div className="">
          <Link to="/">
          <button type="button" className={`btn btn-light checkout-button cont-button mb-2 mt-2`} style={{
          ':hover': {
          ...
          <div className="">
          <Link to='/context/orderform'>
            <button type="button" className="checkout-button" onClick={handleChangeOfbtn}>
              PROCEED TO CHECKOUT
            </button>
          </Link>
        </div>
        <div className="row gt">
          <p className="d-flex justify-content-center mt-3 OrderRes">
            Hira G's process all orders in PKR. While the content of your cart is currently displayed in PKR,
            you will check out using PKR at the most current exchange rate.
          </p>
        </div>
      </div>
